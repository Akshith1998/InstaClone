...
const Form = () => {
    ...
    const handlesubmit=(e)=>{
        ...
        axios.post("https://instaclone-akshith-server.herokuapp.com/post",formData,{headers:{ "Content-Type": "multipart/form-data" }}).then((data)=>{
            console.log(data);
        }).catch((error)=>{
            console.log(error);
        })
        ...
        <div className="form">
            <form>
                <div className="first-row">
                    <span>{photo.name===undefined ? "No File Choosen":photo.name}</span>
                    <label className="file">
                        <input type="file" id="photo" name="photo" accept="image/png,image/jpeg" onChange={(e)=>setPhoto(e.target.files[0])}/>
